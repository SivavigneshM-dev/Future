...
function buildZodiacGrid() {
    ...
    zodiacData.forEach(zodiac => {
        ...
        option.onclick = () => {
            document.querySelectorAll('.zodiac-option').forEach(opt => opt.classList.remove('active'));
...
function setupEventListeners() {
    document.getElementById('firstName')?.addEventListener('input', generateRandomSecretQuestion);
    document.getElementById('lastName')?.addEventListener('input', generateRandomSecretQuestion);
    document.getElementById('dob')?.addEventListener('change', generateRandomSecretQuestion);
    document.getElementById('fatherName')?.addEventListener('input', generateRandomSecretQuestion);
    document.getElementById('motherName')?.addEventListener('input', generateRandomSecretQuestion);
    document.getElementById('verifySecretBtn')?.addEventListener('click', verifySecretAndProceed);
    document.getElementById('unlockScratchBtn')?.addEventListener('click', unlockScratchWithCode);
    document.getElementById('shareBtn')?.addEventListener('click', shareFoolMessage);
    document.getElementById('closeModalBtn')?.addEventListener('click', closeScratchModal);
}
...
function addSibling() {
    const type = prompt('Select sibling type:\n1. Younger Brother\n2. Elder Brother\n3. Younger Sister\n4. Elder Sister', 'Younger Brother');
    const name = prompt('Enter sibling name:');
    if (!name) return;

    let siblingType = '';
    if (type === '1' || type === 'Younger Brother') siblingType = 'Younger Brother';
    else if (type === '2' || type === 'Elder Brother') siblingType = 'Elder Brother';
    else if (type === '3' || type === 'Younger Sister') siblingType = 'Younger Sister';
    else if (type === '4' || type === 'Elder Sister') siblingType = 'Elder Sister';
...
function generateRandomSecretQuestion() {
    ...
    const zodiacPos = zodiacData.find(z => z.name === selectedZodiac)?.position || 5;
    ...
    if (currentQuestionType === 'a') {
        ...
        const motherLetters = motherName.length >= 3 ? motherName.substring(1, 3) : motherName.substring(0, 2);
    ...
    document.getElementById('generatedCodeDisplay').innerHTML = `🔑 Generated Secret Code: <strong>${currentSecretCode}</strong>`;
}
...
function unlockScratchWithCode() {
    const enteredCode = document.getElementById('scratchCodeInput').value.trim().toLowerCase();
...
function showFoolPopup() {
    ...
    popup.innerHTML = '🎭 Share this to Fool your friends! 🎭 <button id="closePopupBtn" style="margin-left:8px;background:none;border:1px solid #1e1a2f;border-radius:20px;padding:2px 8px;cursor:pointer;font-weight:bold;">✖</button>';
